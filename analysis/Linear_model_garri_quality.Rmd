...
packages_used <- c("tidyverse", "ggplot2", "here", "stringr" , "sommer","lme4","metan", "rrBLUP","FactoMineR","corrplot")
...
for (package in packages_used){
  if (!(package %in% ip[,"Package"])){
    ...
    library(package,
  character.only = T) # load required package
  } # end else statement
}#END packages_used
...
dim(chem_func)
str(chem_func)
chem_func$Location <-as.factor(chem_func$Location)

chem_func$Genotype <-as.factor(chem_func$Genotype)
...
```{r use the data with amylose and crude fiber for the lmer and broad sense heritability}
...
```{r}
umu_hav_2020 <-read.csv("/Users/ca384/Documents/ChinedoziRepo/GarriQuality/data/2020_Umu_pyt_harvest_dmc.csv")
colnames(umu_hav_2020)

oto_hav_2020 <- read.csv("/Users/ca384/Documents/ChinedoziRepo/GarriQuality/data/2020_0t0bi_pyt_harvest_dmc.csv")
colnames(oto_hav_2020)

umu_hav_2020<- umu_hav_2020 %>% 
       rename(year = years) 
Umu_Oto_2020_hvst <- rbind(umu_hav_2020,oto_hav_2020)
```
