...
packages_used <- c("workflowr", "tidyverse", "ggplot2", "here", "stringr" , "sommer","lme4","metan", "rrBLUP","FactoMineR","corrplot", "ASRgenomics", "reshape2")
...
Umu_22_func <- Umu_22_func %>% 
      dplyr::rename(Bulk.Density = AverageBD) # rename bulk density

Umu_22_func <- Umu_22_func %>% 
        dplyr::rename(WAC= wac.) # rename wac. to WAC
Umu_22_func <- Umu_22_func %>% 
       dplyr:: rename(swelling.index= Swelling.index) # rename wac. to WAC

Oto_22_func <- Oto_22_func %>% 
        dplyr::rename(WAC= wac.) # rename wac. to WAC for otobi
```
...
Oto_19 <- Oto_19 %>% 
        dplyr::rename(Genotype= sample_id)

Umu_19 <- Umu_19 %>% 
        dplyr::rename(Genotype= Genotypes)

Umu_19 <- Umu_19 %>% 
        dplyr::rename(Moisture = X.moisture)

Umu_19 <- Umu_19 %>% 
        dplyr::rename(Dry.Matter =X..Dry.Matter)

Oto_19 <- Oto_19 %>% 
        dplyr::rename(swelling.index=Swelling.Index..final.initial.)

Oto_19 <- Oto_19 %>% 
        dplyr::rename(swelling.power= SP)

Oto_19 <- Oto_19 %>% 
        dplyr::rename(Bulk.Density= Bulk.Density..g.ml.)
...
Umu_21 <- Umu_21 %>% 
        dplyr::rename(Genotype= Genotypes) # rename Genotypes to genotype
Umu_21 <- Umu_21 %>% 
        dplyr::rename(swelling.index= Swelling.index)
...
chem_pheno <- dplyr::full_join(chem_func, starch_sugar, by=c("Location", "Year", "Genotype")) #. changed chem_func2 that was initially there to chemfunc
...
```{r script to clean up the snps file using the ASRgenomics}
...
#the best loop it contains lmer, mmer, boxplot, 
...
write.csv(x =vcomp, file = "~/Documents/ChinedoziRepo/GarriQuality/output/Variance _comp_heritab_3yrs.csv")
...
#runninng linear model for only 2021 and 2022 data
```{r function to loop all mixed models using the GA matrix for all traits using data without outliers for only 2021 and 2022 data, also made the boxplot of the physicochemical in a loop}
unique(Dat_no_outlier3$Year)
New_data = droplevels(Dat_no_outlier3[Dat_no_outlier3$Year!= 2020,]) # exclude 2020 data

New_data<-New_data %>% 
  dplyr::rename(STC = STA , SC= SUG)
 
Data6= New_data

traitnames<- (c("SI","SP", "WAC", "BD", "AMY","CF", "STC", "SC"))
...
#Dat_no_outlier3 = Dat_no_outlier2[idg,]# row numbers 
#all(Dat_no_outlier3$Genotype %in% rownames(Gmat1))# to check if data have both phenotype and genotype
#traitnames = traitnames[-7]
for(traits in traitnames){
 trs = paste0(traits) # the loop will be able to select  and analyze each trait
 out_ind <- which(Data6[,paste(traits)] %in% boxplot.stats(Data6[,paste(traits)])$out) # outliers

 if(length(out_ind) == 0){
 Data7 = Data6
 }else{
 Data7 = Data6[-out_ind,] # detect outliers
 }
 eval(parse(text=paste(
  "model1= mmer(",traits,"~ 1,
     random= ~Year  + Location + Year:Location + vsr(Genotype, Gu=Gmat1) +  Genotype:Year,
...
vp =  vg + vgy/2  + ve/4
h = round(vg/vp,3)
tt = cbind("Trait" = paste(traits), "Method" = "mmer", "vg" = round(vg,3), "vgy" = round(vgy,3), "vph" = round(vp,3), "ve" = round(ve,3), "H2" = h)
...
eval(parse(text=paste(
  "model2= lmer(",traits,"~ (1|Year) + (1|Location) + (1|Year:Location) 
      + (1|Genotype) + (1|Genotype:Year),
  data=Data7)")))
...
#vgl1 = ss1[ss1$grp == "Genotype:Location","vcov"]
ve1 = ss1[ss1$grp == "Residual","vcov"] 
vp1 =  vg1  + (vgy1/2)  + (ve1/4) 
h1 = round(vg1/vp1,3)

tt1 = cbind("Trait" = paste(traits), "Method" ="lmer","vg" = round(vg1,3), "vgy" = round(vgy1,3), "vph" = round(vp1,3), "ve" = round(ve1,3), H2 = h1)

vccomb = rbind(tt,tt1)
vcomp = rbind(vcomp,vccomb) # this bind the variance components to the functional properties

Bx_1 <- Data7[,traits]
Bx_2 <- cbind(Year=as.character(Data7$Year), Location=as.character(Data7$Location),Geno = as.character(Data7$Genotype), trait=paste0(traits), value=Bx_1)
filtered_value= rbind(filtered_value, Bx_2)
}

# long format
vcomp
 #making a boxplot with all the years, locations  and traits a
filtered_value$value=as.numeric(filtered_value$value)
  global_size=8
ggplot(filtered_value, aes(x = Location, y=value, fill=Location))+
...
# changing the for correlation. using the mean to make correlation
filtered_value$value = as.numeric(filtered_value$value)
...
write.csv(x =vcomp, file = "~/Documents/ChinedoziRepo/GarriQuality/output/Variance _comp_heritability_2yrs_garri.csv")
...
```{r, Not so important: Looking at the distribution of starch using ggplot}
ggplot(Data6,aes(x = Genotype, y= STC)) + 
  geom_boxplot() + facet_grid(Year~Location )
model <- lm(formula = STC~ Year + Location + Year:Location + Genotype + Genotype:Year + Genotype:Location, data= Data6)
anova(model)
library(emmeans)
emmeans(object = model,specs = "Genotype") # looking for means
```

# genomic predictions for dataframe without 2020 data
```{r, genomic prediction for the chemical_functional properties of garri}
set.seed(123)
Acc_pred <- tibble()
traits<- (c("SI","SP", "WAC", "BD", "AMY","CF", "STC", "SC"))
...
for(Trait in traits){
  eval(parse(text = paste("ansm <- lmer(",Trait,"~ (Genotype) + Location +
    (1|Year) ,
      data= Data7)"))) # we used the main data for the GS model
  em = emmeans(object = ansm, specs = ~ Genotype) # get the output means of the genotype
  em = as.data.frame(em)
  m2 = cbind(Trait = paste(Trait), em[,1:2])
  mean_Data7 = rbind(mean_Data7,m2) 

}

library(reshape)
Meanwide1 <-dcast(data = mean_Data7, formula = Genotype ~ Trait, fun.aggregate = mean,
                 value.var = "emmean") # change the format of the mean_Data7
...
for(Trait in traits) {
  ...
  fold5 = caret::createMultiFolds(y = unique(Meanwide1$Genotype), k = 5, times = 5)


  for(i in 1:length(fold5)){
    index = fold5[[i]] # the index of the sample for training set
    #subset the phenotypic data, 
    train_geno <- droplevels(unique(Meanwide1$Genotype)[index])
    train_geno_ind <- which(Meanwide1$Genotype %in% train_geno)
    train.data <- droplevels(Meanwide1 %>%
                               filter(row_number() %in% train_geno_ind)) # subset the training set
    dim(train.data)
    test.data <- droplevels(Meanwide1 %>%
    ...
    mod_dat1<- rbind(train.data, test.data) # combine the the data set for analysis
...
```{r, genomic prediction for the chemical_functional properties of garri using genomicMate}
...
# library(sommer)
# BLUPS_traits = tibble()
# traitnames1 = traitnames[!traitnames %in% "CF"]
# for(Trait in traitnames1){
#   
# eval(parse(text = paste( "ansb <- mmer(fixed = ",Trait,"~ Year + Location, 
#              random = ~ Genotype,
#              rcov = ~units,
#              getPEV = TRUE,
#              data= Data7)")))
# 
# Vg = ansb$sigma$Genotype
# ve = ansb$sigma$units
# H2 = Vg/(Vg +(ve/(3*2)))
# blups<-ansb$U$Genotype[[Trait]] %>% 
#      tibble(Genotype=names(.),BLUP=.) %>% 
#      mutate(Genotype=gsub("Genotype","",Genotype),
#             # prediction error variance
#             PEV=diag(ansb$PevU$Genotype[[Trait]]), 
#             
#             REL=1-PEV/Vg, # Reliability
#             drgBLUP=BLUP/REL, # De-regressed BLUP
#             WT=(1-H2)/((0.1 + (1-REL)/REL)*H2))
# blups1 = cbind(Trait = Trait, blups)
#  BLUPS_traits = rbind(BLUPS_traits, blups1)           
# 
# }
# 
# BLUPS_traits[BLUPS_traits$Trait == "CF",]
# idmt = which(rownames(Gmat1) %in%  blups$Genotype)
# Gmat_dg = Gmat1[idmt, idmt]
# all(rownames(Gmat_dg) %in% blups$Genotype)
#       
...
# #                 random = ~vs(Genotype,Gu=Gmat_dg),
# #                 weights = WT,
# #                 data=blups) # use for later analysis
# #   
# # r = cor(gblup_all$U$`u:Genotype`$drgBLUP, blups$drgBLUP)      
#       
#       
# 
# 
...
# head(BLUPS_traits)
# for(Trait in traitnames1){
# blups <- BLUPS_traits[BLUPS_traits$Trait == Trait,]
#   fold5 = caret::createMultiFolds(y = unique(blups$Genotype), k = 5, times = 5)
# 
# 
#   for(i in 1:length(fold5)){
#     index = fold5[[i]] # the index of the sample for training set
#     #subset the phenotypic data, 
#     train_geno <- unique(blups$Genotype)[index]
#     train_geno_ind <- which(blups$Genotype %in% train_geno)
#     train.data <- droplevels(blups %>%
#                                filter(row_number() %in% train_geno_ind)) # subset the training set
#     dim(train.data)
#     test.data <- droplevels(blups %>%
#                               filter(!row_number() %in% train_geno_ind)) # subset the testing set
#     dim(test.data)
#     test.data1 <- test.data
#     test.data[,"drgBLUP"] = NA # change the grain yield of the training set to NA value
# 
#     mod_dat1<- rbind(train.data, test.data) # combine the the data set for analysis
# 
#     #####################
# 
# 
#     eval(parse(text = paste("ans4<-mmer(fixed = drgBLUP~1,
#             random = ~vs(Genotype,Gu=Gmat_dg),
#                 weights = WT,
#                 data=mod_dat1) # use for later analysis   ")))
# 
#     pblup = as.data.frame(ans4$U$`u:Genotype`)
#     pblup = cbind(rownames(pblup),pblup)
#     colnames(pblup)[1] = "Genotype"
...
# 
#     obs_test =  test.data1 %>% group_by(Genotype) %>%
#       summarise_at(.vars = "drgBLUP", .funs = mean)
#     obs_test= obs_test[order(obs_test$Genotype),]
#     r = cbind( Trait = Trait, predictability = round(cor(testpred[,"drgBLUP"],obs_test[,"drgBLUP"], use = "pairwise.complete.obs"),3))
#     colnames(r)[2] <- c("predictability")
#     Acc_pred = rbind(Acc_pred,r)
# 
# 
...
```{r model of two years data with the full model}
traitnames<- (c("SI","SP", "WAC", "BD", "AMY","CF", "STC", "SC"))
...
for(traits in traitnames){
 ...
 eval(parse(text=paste(
  "model1= mmer(",traits,"~ 1,
     random= ~Year  + Location + Year:Location + vsr(Genotype, Gu=Gmat1) +  Genotype:Year + Genotype: Location,
     rcov=~units, tolParInv = 0.1,
  data=Data7)")))
...
tt1 = cbind("Trait" = paste(traits), "Method" ="lmer","vg" = round(vg1,3), "vgl"=round(vgl1,3), "vgy" = round(vgy1,3), "vph" = round(vp1,3), "ve" = round(ve1,3), H2 = h1)
...
# long format
vcomp
write.csv(x=vcomp,file = "~/Documents/ChinedoziRepo/GarriQuality/output/H2_garri_full_model.csv")
```
```{r, Predict the BLUEs for garri to to make a biplot for the physicochemical traits of garri}

library(emmeans)
traitnames<- (c("SI","SP", "WAC", "BD", "AMY","CF", "STC", "SC"))
BLUEMean = tibble()
for(traits in traitnames){
eval(parse(text = paste("model1= lmer(",traits,"~ (1|Year)  + (1|Location) + (1|Year:Location) + Genotype +  (1|Genotype:Year) + (1|Genotype: Location),
  data=Data7)")))
 em1 = as.data.frame(emmeans(object = model1, specs = "Genotype"))
...
# pc1 = PCA(X = BLUEMeanWide[,-1], scale.unit = T  )
# factoextra::fviz_pca_ind(pc1,  labelsize = 1)
# fviz_pca_biplot(X = pc1, labelsize =1,arrowsize = 0.2, pointsize = 0.5, col.var = "red")
...
head(BLUEMean)
 ...
 BLUEMeanWide$Gcode = gc
 rownames(BLUEMeanWide) <- BLUEMeanWide$Gcode
 pc_G = PCA(X = BLUEMeanWide[,-c(1,10)], scale.unit = T  )
 factoextra::fviz_pca_ind( pc_G,cex=0.8,  labelsize = 1)

fviz_pca_biplot(X = pc_G,labelsize= 2, arrowsize = 0.3,
                 pointsize = 0.1, col.var = "red") +
   theme(text = element_text(size = 12))

 write.csv(x = BLUEMeanWide[,c(1,10)], file = "/Users/ca384/Documents/ChinedoziRepo/GarriQuality/output/Garri_genotype_code.csv",row.names = F)
...
# library(biotools)
# 
# garri_mano = manova(cbind(SI,SP, WAC, BD, AMY,CF, STC, SC)~Year + Location + Year:Location +
#       Genotype + Genotype:Year + Genotype:Location,
#   data=Data7)
# garri_gencov <- gencovtest(obj = garri_mano,geneticFactor = "Genotype")
# diag(garri_gencov$gcov)
# garri_gencov$gcor
# #####
# # sommer
# 
# mrc1 = mmer(fixed = cbind(WAC, SC) ~ 1, 
#      random = ~ Year:Location +
#        Genotype + Genotype:Year + Genotype:Location, 
#      rcov = ~units, data = Data7)
# ss = summary(mrc1)
# ss$varcomp
# 2.374/(sqrt(69.8*0.00231))
# for(i in traitnames){
#   for(j in traitnames){
#     
# mod1= lmer(WAC~ (1|Year) + (1|Location) + (1|Year:Location) +
#        (1|Genotype) + (1|Genotype:Year) + (1|Genotype:Location),
#   data=Data7)
# mod2= lmer(WAC~ (1|Year) + (1|Location) + (1|Year:Location) +
#        (1|Genotype) + (1|Genotype:Year) + (1|Genotype:Location),
#   data=Data7)
# modlxy = lmer(WAC~ (1|Year) + (1|Location) + (1|Year:Location) +
#        (1|Genotype) + (1|Genotype:Year) + (1|Genotype:Location),
#   data=Data7)
# 
#  }
