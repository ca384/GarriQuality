...
traitname<- (c("swelling.index","swelling.power", "WAC", "Bulk.Density", "Amylose","Crude.Fiber")) # matrix of column names
Result.h2 = matrix(nrow = 1, ncol = length(traitname)) # this is how we want the result of the heritability to appear after analysis
colnames(Result.h2) <- traitname # naming the colnames of the heritability output so we can identify which trait has that heritability
rownames(Result.h2) <- "H2Comb" # heritability for the combine analysis
Result.vcomp = matrix(nrow = length(traitname), ncol = 5) # Result matrix for the variance components
...
for(trait in traitname){
tr = paste0(trait)  # the loop will be able to select  and analyse each trait
Na_g = which(is.na(chem_func2[,tr]))# identify traits with missing data
if(length(Na_g) == 0){                 # to see if there is any null within the length within the length
DatWithout_na = chem_func2    # if there is no na, it will use the pyt_functional_1 data frame
}else{
DatWithout_na = chem_func2[-Na_g,]  # if there is missing data in the data frame, the use a new dataframe pyt_functional_1[-Na_g,] that has all the na removed
}
...
ggplot(data = Dat_no_outlier,aes(x=Year,y= swelling.index,fill=Location)) +
   geom_boxplot() 
ggplot(data = Dat_no_outlier,aes(x=Year,y= swelling.power,fill=Location)) +
  geom_boxplot()
ggplot(data = Dat_no_outlier,aes(x=Year,y= WAC,fill=Location))+
   geom_boxplot()
ggplot(data = Dat_no_outlier,aes(x=Year,y= Bulk.Density,fill=Location)) +
  geom_boxplot()
 
ggplot(data = Dat_no_outlier,aes(x=Year,y= Amylose,fill=Location)) +
  geom_boxplot()

ggplot(data = Dat_no_outlier,aes(x=Year,y=Crude.Fiber,fill=Location)) +
  geom_boxplot()
...
table(rownames(snps_dosage) == chem_func2$Genotype)

table(chem_func2$Genotype == rownames(snps_dosage) )
...
# gen = which(rownames(snps_dosage) %in%pyt_functional_1$Genotype ) # 
# Subset the pheno data that has snp data 
gt2 = which(chem_func2$Genotype %in% rownames(snps_dosage) ) # identify the row number/genotype name those gentypes from pheno to snps

phen2 = droplevels(chem_func2[gt2,]) # subset the pheno data that found in the snp data

head(phen2)
dim(phen2)
dim(chem_func2)
length(rownames(snps_dosage)) # length row name of the snps
length(levels(as.factor(phen2$Genotype))) # the length of the selected pheno data genotype levels 

# if the legnths are equal no need subseting the snp data
# but if the length is different we should subset the snp data in relation to the selected pheno genotypes
library(ASRgenomics)
gsnp2 = which(rownames(snps_dosage) %in% levels(phen2$Genotype)) 

snp3 = snps_dosage[rownames(snps_dosage)[gsnp2],] # subset the snps based on the genotypes with snp and also phenotype

gph2 = which(levels(phen2$Genotype) %in% rownames(snps_dosage))# looking for phenotype data in snps_dosage file.
phen3 = phen2[gph2, ]
all(phen3$Genotype %in% rownames(snp3))
length(levels(phen3$Genotype))
all(rownames(snp3) %in% levels(phen2$Genotype))
```
...
snp2 <- snp2[,-1]
snp2 <- as.matrix(snp2)
snpsFilter  <- qc.filtering(M=snp2, base=FALSE, ref=NULL,
...
for(trait in traitname){
  ...
  if(length(Na_g) == 0){                 # to see if there is any null within the length within the length
    ...
    DatWithout_na2 = chem_func2[-Na_g,]  # if there is missing data in the data frame, the use a new dataframe pyt_functional_1[-Na_g,] that has all the na removed
  }
...
```{r function to loop all mixed models using the GA matrix for all traits}
traitnames<- (c("swelling.index","swelling.power", "WAC", "Bulk.Density", "Amylose","Crude.Fiber"))
...
eval(parse(text=paste(
  ...
  random=~vsr(Genotype, Gu=Gmat) + Genotype:Year + Genotype:Location,
  rcov=~units,
  data= Dat_no_outlier2)")))
