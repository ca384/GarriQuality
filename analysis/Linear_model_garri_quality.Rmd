...
packages_used <- c("tidyverse", "ggplot2", "here", "stringr" , "sommer","lme4","metan", "rrBLUP","FactoMineR")
...
chem_func1 <- chem_func %>% dplyr::select(c("swelling.index","swelling.power","WAC","Bulk.Density","Amylose","Crude.Fiber"))
```
...
library(corrplot)
cor_chem_func <- corrplot(corr, method = "circle",type = "full",is.corr = TRUE)
 df2 <- df %>% select(-c(id, name, chapters))   corrplot(corr, order = 'alphabet', addCoef.col = 'black') 
